...
def train_model_transfer(config,trans_rundir):
    ...
    trainer, epoch = Trainer.load_transfer(config, vocab, last=load_last,trans_rundir=trans_rundir)
    trainer.train(dataset.get_loader(), 0,config)
...
if __name__ == "__main__":
    ...
    if command == 'preprocess':
        ...
        config = Config(args.pop('dataset'), **args)
        trans_rundir = "./RUNS/XXX-ZINC/ckpt/best_valid.pt"
        train_model_transfer(config,trans_rundir)
